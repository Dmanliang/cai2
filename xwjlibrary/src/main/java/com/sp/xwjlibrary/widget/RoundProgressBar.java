...
public class RoundProgressBar extends View {
    ...
    public void setOnProgressChangeListener(OnProgressChangeListener changeListener){
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        //文字的单位
        textUnit = mTypedArray.getString(R.styleable.RoundProgressBar_txtUnit);
        //圆环的宽度
        roundWidth = mTypedArray.getDimension(R.styleable.RoundProgressBar_roundWidth, 5);
        //最大进度
        max = mTypedArray.getInteger(R.styleable.RoundProgressBar_max, 100);
        //是否显示中间的进度
        textIsDisplayable = mTypedArray.getBoolean(R.styleable.RoundProgressBar_txtIsDisplayable, true);
        //进度的风格，实心或者空心
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float textWidth = paint.measureText(percent + textUnit);   //测量字体宽度，我们需要根据字体的宽度设置在圆环中间

        if (changeListener != null){
            changeListener.onProgressChange(percent);
        }
        if(textIsDisplayable && percent != 0 && style == STROKE){
            canvas.drawText(percent + textUnit, centre - textWidth / 2, centre + textSize/2, paint); //画出进度百分比
        }
